...
internal class ServiceRepository : IServiceRepository
{
    ...
    public async ValueTask AddAsync(Service entity, CancellationToken cancellationToken = default)
    {
        var service = _mapper.Map<ServiceDataModel>(entity);
        await _services.InsertAsync(service);
    }

    public async ValueTask<Service?> FindAsync(ServiceId id, CancellationToken cancellationToken = default)
    {
    ...
    public async ValueTask UpdateAsync(Service entity, CancellationToken cancellationToken = default)
    {
        var service = _mapper.Map<ServiceDataModel>(entity);
        await _services.UpdateAsync(service);
    }

    public ValueTask DeleteAsync(ServiceId id, CancellationToken cancellationToken = new CancellationToken())
    {
